...
var Volcanos = shy({iceberg: "", volcano: "", frame: chat.FRAME_JS, _cache: {}, cache: {}, pack: {}, args: {}}, function(name, can, libs, cb) {
	...
	can = kit.proto(can||{}, kit.proto({_name: name, _path: _can_name,
		...
		require: function(libs, cb, cbs) {
			...
			if (name.indexOf("/m/") == 0 && meta.iceberg) { name = meta.iceberg+name }
		...
		ConfCmd: function(msg) { msg = msg||can._msg; return (msg.Option("_input_cmd")||msg.Option("_other_cmd")||can.ConfIndex()).split(".").pop() },
		...
		ConfHelp: function() { return can.Conf(mdb.HELP) },
