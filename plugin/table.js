Volcanos(chat.ONIMPORT, {
		...
		can.page.Append(can, target, msg.Table(function(value) { if (filter && filter(value)) { return }
			...
			return {view: [[html.ITEM, value.type, value.status, "s-"+value.name]], list: [
				{view: ["title", html.DIV], list: [
					...
					{view: wiki.TITLE, list: [value.help && value.name.indexOf(value.help) == -1 && {text: value.help}, {text: value.name}, value.exists == "true" && {text: ["●", "", "exists"]}]},
			...
			can.onimport.plug(can, meta, function(sub) {
				...
				sub._legend.oncontextmenu = sub._legend.onclick, sub._legend.onclick = function(event) { can.misc.Event(event, can, function(msg) {
					if (can.page.SelectOne(can, status, "legend.select", function(target) {
						can.onmotion.shakeOut(can, target._target, function() {
	...
	myView: function(can, msg) { can.onappend.table(can, msg) },
	myViewTabs: function(can, tabs, msg) { can.onappend.table(can, msg) },
})
...
Volcanos(chat.ONACTION, {
	...
	onslidedown: function(event, can, data, direction) { var target = can.ui.list||can.ui.output||can._output
		if (target.scrollTop < -100) {
			can.Update(can.request(event, {_toast: "reload"}))
		}
	},
	onslideup: function(event, can, data, direction) { var target = can.ui.list||can.ui.output||can._output
...
Volcanos(chat.ONINPUTS, {
	_nameicon: function(event, can, msg, target, name, title, icons) { name = name||mdb.NAME
		can.page.Appends(can, can._output, msg.Table(function(value) {
			...
			return {view: html.ITEM, list: [{icon: _icons}, {view: html.CONTAINER, list: [
				{view: [html.TITLE, "", _title]}, can.onappend.label(can, value, kit.Dict(
					"version", icon.version, "time", icon.compile, name, icon.data, "user_name", icon.username,
				)),
