Volcanos(chat.ONFIGURE, {video: {
	_init: function(can, meta, target) { target.value == meta.value && (target.value = ""); var images = can.core.Split(target.value)
		var count = parseInt(meta.value||"9"), width = target.parentNode.offsetWidth-2; for (var n = 1; n < 10; n++) { if (n*n >= count) { width = (width/n); break } }
		...
		function add(target, hash) { target._hash = hash, can.page.Appends(can, target, [can.page.AppendVideo(can, hash)]) }
		...
		function push(p) {
			...
			can.page.Append(can, target.parentNode, [{view: html.FLEX, style: {height: count > 1? width: "", width: count == 1? target.parentNode.offsetWidth: width}, _init: function(target) {
