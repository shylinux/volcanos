Volcanos("base", {
	...
	_parse: function(url, res) {
		var list = url.split("#")[0].split(ice.QS); res = res||{}, res._origin = list[0]
		list[1] && list[1].split("&").forEach(function(item) { var ls = item.split(mdb.EQ); res[decodeURIComponent(ls[0])] = decodeURIComponent(ls[1]) })
		var args = url.split("#")[1]; args && (res.args = args.split(":"))
		return res
	},
	...
	ParseURL: function(url) {
		var res = this._parse(url); res.link = url, res.origin = res._origin; return res
	},
	...
	TimeTrim: function(value) { if (!value) { return } var prefix = ""
		var now = new Date(), year = now.getFullYear()+"-", pre = this.Time(now, "%y-%m-%d ")
		if (value.indexOf(pre) == -1) { var list = ["昨天", "前天", "", "", "", "", ""]
			for (var i = 0; i < list.length; i++) { var yestoday = this.Time(new Date(now - (i+1)*24*60*60*1000), "%y-%m-%d ")
		...
		if (value.indexOf(year) == -1) { return value.split(" ")[0] }
