Volcanos("page", {
	...
	Modify: function(can, target, value) { target = can.base.isString(target)? document.querySelector(target): target; if (!target) { return }
		can.base.isString(value)? (target.innerHTML = value): can.core.Item(value, function(key, val) { if (key == "view") { return }
			key == "className" && can.base.isArray(val) && (val = val.join(lex.SP)), !can.base.isObject(val)? (target[key] = val): can.core.Item(val, function(k, v) {
				...
				if (can.base.isIn(k, "height", "width", "min-height", "max-height", "min-width", "max-width", "left", "top", "right", "bottom", "margin-left", "margin-top", "margin", "padding", "font-size") && v && (can.base.isNumber(v) || (!can.base.endWith(v, "px") && !!can.base.endWith(v, "%")))) { v += "px" }
				if (can.base.isIn(k, "background-image") && v) { v = "url("+v+")" }
		...
		value = value||{}, can.core.List(key, function(item) { if (!item) { return } if (item.nodeName) { target.appendChild(item); return }
			if (can.base.isString(item)) {
				if (can.base.beginWith(item, "<")) {
					var what = document.createElement("div"); what.innerHTML = item, target.appendChild(what.children[0]); return
				}
				item = {view: [item]}
			}
			...
			if (item.view) {
				var list = can.core.List(item.view);
				if (can.base.isArray(list[0])) {
				...
				var list = can.core.List(item.link)
				type = "a"
				data.href = data.href||list[0]
				data.target = "_blank"
				data.innerText = data.innerText||list[1]||data.href
				...
				if (icon[item.icon] || can.base.beginWith(item.icon, "bi bi")) {
					type = "i", data.className = icon[item.icon]||list[0]
				...
				} else if (can.base.beginWith(item.icon, "http", "/p/")) {
					type = "img", data.src = item.icon
				} else {
					type = "i", data.className = list[0]
				}
			} else if (item.avatar) {
				var list = can.core.List(item.avatar)
				if (icon[item.avatar] || can.base.beginWith(item.avatar, "bi bi")) {
					type = "i", data.className = icon[item.icon]||list[0]
				} else {
					type = html.IMG, data.src = can.misc.ResourceIcons(can, list[0]), data.className = data.className||"avatar"
				}
				...
				var list = can.core.List(item.video); type = html.VIDEO, data.src = list[0]
			} else if (item.img) {
	...
	AppendVideo: function(can, p) {
		if (can.user.isMobile) {
			return "<video class='select' src='"+can.misc.Resource(can, p)+"' autoplay loop playsinline webkit-playsinline></video>"
		} else {
			return "<video class='select' src='"+can.misc.Resource(can, p)+"' autoplay loop controls playsinline webkit-playsinline></video>"
		}
	},
	...
	AppendTable: function(can, msg, target, list, cb) { if (!msg.append||msg.append.length == 0) { return }
		...
		can.page.Append(can, ui.thead, [{type: html.TR, data: {dataset: {index: -1}}, list: can.core.List(list, function(key) {
			if (key[0] != "_") { return {className: option.indexOf(key) > -1 && can.Option(key) == ""? ice.MSG_OPTION: key == ctx.ACTION? ctx.ACTION: "", type: html.TH, list: [{text: can.user.trans(can, key, null, html.INPUT)}, {icon: "bi bi-sort-down-alt"}, {icon: "bi bi-sort-up"}]} }
		}) }])
	...
	Color: function(text) { if (typeof text != code.STRING) { return "" } text = text.replace(/\\n/g, "<br>")
		...
		text = text.replace(/\033\[31m/g, "<span style='color:#cc0000'>")
		text = text.replace(/\033\[32m/g, "<span style='color:#4e9a06'>")
	...
	insertBefore: function(can, target, before, parent) { if (!target) { return }
	...
	inputs: function(can, list, type) {
		var _list = []; for (var i = 0; i < list.length; i++) {
			switch (list[i]) {
				...
				case ice.AUTO: _list.push({type: html.BUTTON, name: ice.LIST}), _list.push({type: html.BUTTON, name: ice.BACK}); break
				case web.FILTER: _list.push({type: html.TEXT, name: web.FILTER, icon: icon.search}); break
				case mdb.PAGE:
				...
				_list.push({type: html.TEXT, name: mdb.LIMIT, value: can._msg.Option(mdb.LIMIT), _init: function(target) {
					can.onappend.figure(can, {action: mdb.KEY, run: function(event, cmds, cb) {
						var msg = can.request(event); msg.Push(cmds[1], "10"), msg.Push(cmds[1], "30"), msg.Push(cmds[1], "50"), msg.Push(cmds[1], "100"), cb(msg)
					}}, target, function() { can.Update() })
				}}), _list.push(mdb.NEXT, mdb.PREV); break
	...
	button: function(can, name, cb) {
		...
		return {view: [["item.button", data.className||data.name]], list: [
	...
	appendAction: function(can, value, target) { if (!value.action) { return } target.innerHTML = value.action
		can.page.Select(can, target, html.INPUT_BUTTON, function(target) {
			...
			target.onclick = function(event) {
				...
				can.onkeymap.prevent(event), can.Update(can.request(event, value), [ctx.ACTION, target.name])
			}
		})
	},
	...
	buttonStyle: function(can, name) {
		...
		can.core.Value(can.onaction, ["_trans", "style", name])||(can.base.isIn(name, mdb.CREATE, mdb.INSERT, mdb.IMPORT, nfs.CLONE, cli.START, ctx.RUN, web.UPLOAD, web.CONFIRM, aaa.LOGIN, "sso", "add", "pull", "push", "submit", "commit", "preview", "auto-preview", ice.APP)? html.NOTICE:
