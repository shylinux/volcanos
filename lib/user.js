Volcanos("user", {
	...
	toast: function(can, content, title, duration, progress, hash) { can = can._fields? can.sup: can
		...
		var action = can.onappend._action(can, meta.action.list? meta.action.list: meta.action, ui.action, {_trans: meta.action.meta? meta.action.meta._trans: {},
			...
			close: function() {
				function remove(target) { can.page.ClassList.del(can, target, "shake_done")
					can.onmotion.delay(can, function() { can.page.Remove(can, target) }, 300)
				}
		...
		can.onmotion.delay(can, function() { can.page.ClassList.add(can, ui._target, "shake_done") }, 300)
		return action
	},
	...
	carte: function(event, can, meta, list, cb, parent, trans) { var msg = can.request(event); event = msg._event
		...
		function click(event, button, index) {
	...
	input: function(event, can, form, cb, button) { if (!form || form.length == 0) { return cb() }
		...
		var action = can.onappend._action(can, button||[html.SUBMIT, html.CANCEL], ui.action, {
			...
			cancel: function(event, can, button) {
				...
				can.page.ClassList.del(can, ui._target, "shake_done")
				can.onmotion.delay(can, function() {
		...
		can.page.ClassList.add(can, ui._target, "shake_init")
		can.onmotion.delay(can, function() { can.page.ClassList.add(can, ui._target, "shake_done") }, 300)
