Volcanos("user", {
	...
	mod: {
		isPod: location && location.pathname && (
			location.pathname.indexOf("/s/") == 0 ||
			location.pathname.indexOf("/x/") == 0 ||
			location.pathname.indexOf(web.CHAT_POD) == 0
		),
		isCmd: location && location.pathname && (
			location.pathname.indexOf("/c/") >= 0 ||
			location.pathname.indexOf(nfs.WIKI_PORTAL) == 0 ||
			location.pathname.indexOf(web.CHAT_CMD) == 0 ||
			location.pathname.indexOf(web.CHAT_POD) == 0 && location.pathname.indexOf("/cmd/") > 0
		),
	},
	...
	theme: function(can, name) { var list = []
		can.user.mod.isCmd && can.misc.Search(can, "cmd") != "admin" && list.push(chat.CMD)
		var cmd = can.user.mod.cmd||can._root.Action.Conf("tool.0.index"); cmd && list.push(can.base.replaceAll(cmd, ".", " "))
		can.user.language(can) && list.push(can.core.Split(can.user.language(can), "-_.")[0])
		can.user.info.userrole && list.push(can.user.info.userrole)
		can.misc.isDebug(can) && list.push(log.DEBUG)
		...
		can.page.styleClass(can, document.body, list.concat(name).join(lex.SP))
	},
	title: function(text) {
		if (window.webview) { return title(text) }
		return text && (document.title = text), document.title
	},
	favicon: function(can, link) {
		link && can.page.Append(can, document.head, [{type: "link", href: can.misc.Resource(can, link), rel: "shortcut icon"}])
	},
	...
	trans: function(can, text, list, zone) { if (can.base.isNumber(text)) { return text+"" } if (!text || (text.startsWith && text.startsWith("<")) || can.user.isEnglish(can)) { return text }
		...
		can.core.Value(can.user._trans, key) || (text.indexOf("_") > -1 && list? can.user.trans(can, text.split("_").pop(), list, zone):text)
	...
	toastConfirm: function(can, content, title, action) {
		if (!action.list || action.list.length == 0) { action = shy({confirm: action}, [html.CONFIRM, html.CANCEL]) }
	...
	toast: function(can, content, title, duration, progress, hash) { can = can._fields? can.sup: can
		...
		if (can.user.isMobile && !can.user.isLandscape()) { width = window.innerWidth, width = 320 }
		var parent = meta.action.list||meta.action.length > 1 || !can._root||!can._root.Action||!can._root.Action._toast? document.body: can._root.Action._toast
		var ui = can.page.Append(can, parent,
			[{view: [[chat.TOAST, can.ConfIndex(), meta.style, html.FLOAT, parent == document.body? "shake_init": ""]], style: {width: width}, list: [
		...
		var action = can.onappend._action(can, meta.action.list? meta.action.list: meta.action, ui.action, {_trans: meta.action.meta? meta.action.meta._trans: {},
			...
			close: function() {
				function remove(target) {
					can.onmotion.shakeOut(can, target, function() {
						can.page.Remove(can, target)
					})
				}
				can._toast && can._toast._target && remove(can._toast._target)
				can._toast && (typeof can._toast.close == code.FUNCTION) && delete(can._toast)
				remove(ui._target), action.timer.stop = true, delete(can.__toast)
			},
			...
			timer: can.core.Timer({interval: 100, length: (meta.duration||1000)/100}, function(event, interval, index) {
				...
				can.onmotion.delay(can, function() { action.timer.stop || action.close() }, 1000)
			}), _target: ui._target,
		...
		if (meta.hash) { var list = can._root.Action._toastList = can._root.Action._toastList||{}
			try { list[meta.hash] && can.page.insertBefore(can, action._target, list[meta.hash]._target), can.__toast = action } catch (e) {}
		...
		can.onmotion.delay(can, function() {
			can.onmotion.shakeIn(can, ui._target)
			// can.onmotion.shakeOut(can, ui._target)
		}, 300)
		return action
	},
	...
	share: function(can, msg, cmds) { can.page.exportValue(can, msg)
		can.run(msg, cmds||[ctx.ACTION, chat.SHARE], function(msg) { can.user.copy(msg._event, can, msg.Append(mdb.NAME))
			var ui = can.user.toast(can, {style: cli.QRCODE, title: msg.Append(mdb.NAME), duration: -1, content: msg.Append(mdb.TEXT), action: [cli.OPEN, cli.CLOSE], resize: html.IMG})
		})
	},
	...
	carte: function(event, can, meta, list, cb, parent, trans) { var msg = can.request(event); event = msg._event
		...
		function click(event, button, index) { can.request(event, msg)
			can.misc.Event(event, can, function() { can.request(event, {action: button}), can.onkeymap.prevent(event), event._events = _events;
				can.onengine.signal(can, "onevent", can.request(event, {_type: html.ACTION}));
				(can.base.isFunc(cb)? cb(event, button, meta, carte, index): meta[button]? can.core.CallFunc([meta, button], {event: event, can: can, msg: msg, button: button}): can.Update(event, [ctx.ACTION, button]))
				|| can.onmotion.delay(can, function() { can.onmotion.clearCarte(can) }, 300)
			})
		}
	...
	input: function(event, can, form, cb, button) { if (!form || form.length == 0) { return cb() }
		...
		var ui = can.page.Append(can, document.body, [{view: [[html.INPUT, can.Conf(ctx.INDEX)||can.ConfIndex(), msg.Option(ctx.ACTION), msg.Option(mdb.TYPE), chat.FLOAT]], list: [
			{view: html.OPTION, list: [{type: html.TABLE, list: can.core.List(form, function(item) {
				...
				item._init = function(target) {
					...
					if (item.action == "img") { target.value = "" }
					can.onmotion.delay(can, function() {
						can.onappend.figure(can, can.base.Copy({space: msg.Option(web.SPACE), run: function(event, cmds, cb) { var _msg = can.request(event, {_handle: ice.TRUE, action: msg.Option(html.ACTION)}, msg, can.Option())
							can.page.Select(can, ui._target, html.OPTION_ARGS, function(item) { item.name && item.value && _msg.Option(item.name, item.value) }); (item.run||can.run)(event, cmds, cb, true)
				...
				return {view: [[item.name, item.name.split("_").pop(), item.type, item.action], html.TR], list: [
					{type: html.TD, list: [
					...
					{type: html.TD, _init: function(target) {
						var value = msg.Option(item.name)||(can._msg? can._msg.Option(item.name): "")
						if (item.action == "img") { value = "" }
						can.onappend.input(can, item, value, target)
					}},
		...
		var action = can.onappend._action(can, button||[html.SUBMIT, html.CANCEL], ui.action, {
			...
			Option: function(key, value, hidden) {
				...
				var target = can.page.Select(can, ui._target, "input.args[name="+key+"],textarea.args[name="+key+"]")[0]; if (!target) { return }
			...
			cancel: function(event, can, button) {
				...
				can.onmotion.shakeOut(can, ui._target, function() {
					can.page.ClassList.del(can, can._fields||can._target, "_process"), can.onmotion.clearFloat(can), can.page.Remove(can, ui._target)
				})
			},
		...
		can.onmotion.delay(can, function() { action.layout(event), action.focus() }, can.user.isMobile? 0: 300)
		...
		callback("beforeInputs", msg.Option(ctx.ACTION)), can.onmotion.shakeIn(can, ui._target)
	...
	upload: function(event, can, cb, silent) { var begin = new Date()
		...
		var action = can.onappend._action(can, [{type: html.UPLOAD, onchange: function(event) {
			...
			show: function(event, value, total, loaded) {
				...
				ui.show.innerHTML = value+"%", loaded == 0 && action.begin(event)
				ui.size.innerHTML = can.base.Size(loaded)+nfs.PS+can.base.Size(total)
				can.user.toastProcess(can, can.user.trans(can, "upload ", "上传")+files[0].name, "", value)
	...
	login: function(can, _cb, _msg) { can.misc.CookieSessid(can, ""), can.misc.Cookie(can, ice.MSG_SESSID, "")
		...
		function check() {
			if (can.misc.CookieSessid(can) || can.user.info.sessid) { can.page.ClassList.del(can, document.body.parentNode, aaa.LOGIN), can.onimport.theme(can)
				can.page.Select(can, document.body, "div.input.login.float", function(target) { can.page.Remove(can, target) })
		...
		var ui, socket = can.ondaemon._init(can, "", aaa.LOGIN, function(event, msg, cmd, arg) {
			if (cmd == ice.MSG_SESSID) {
				...
				var hassso = false, _list = [], list = {}; _msg.Table(function(value) { if (!value.order) { return } can.user.trans(can, kit.Dict(value.name, value.help))
					if (value.type == web.OAUTH) { hassso = true }
				...
				var _cmd = "space login "+arg[0]; can.user.title(can.user.info.titles)
				...
				can.page.Append(can, ui._target, [{view: [[html.DISPLAY, html.FLEX]], list: [
					...
					{text: ["$ "+_cmd, "", html.ITEM], title: "点击复制，并后台执行此命令，即可登录", style: {cursor: nfs.COPY}, onclick: function() { can.user.copy(event, can, _cmd) }},
					hassso && {text: [can.user.trans(can, "or oauth login", "或第三方授权: "), html.LABEL]},
					hassso && {view: [[chat.SSO, html.FLEX]], list: _msg.Table(function(value) {
						return value.type == web.OAUTH && {view: [[html.ITEM, value.name, html.FLEX]], title: value.link, list: [
	...
	logout: function(can) { can.user.toastConfirm(can, can.user.trans(can, aaa.LOGOUT, "退出登录"), "", function() { can.runAction({}, aaa.LOGOUT, [], function(msg) {
