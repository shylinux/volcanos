Volcanos("user", {
	...
	login: function(can, cb, msg) { can.misc.CookieSessid(can, ""); var socket = can.misc.WSS(can, {type: aaa.LOGIN}, function(cmd, arg) {
		if (cmd == cli.PWD) { if (can.misc.CookieSessid(can)) { return }
		...
		} else if (cmd == ice.MSG_SESSID) { can.page.ClassList.del(can, document.body, aaa.LOGIN)
			if (!can.misc.CookieSessid(can, arg[0])) { can.user.info.sessid = arg[0] }
			can.onmotion.clearFloat(can), can.onmotion.delay(can, function() { socket.close() })
