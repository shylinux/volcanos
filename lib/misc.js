Volcanos("misc", {
	...
	Message: function(event, can) { var msg = kit.proto({}, {_event: event, _can: can, _target: can._target,
		RunAction: function(event, sub, cmds, meta) { var msg = can.request(event); meta = meta || sub&&sub.onaction || {}
			...
			if (cmds && cmds[0] == ctx.ACTION && can.base.isFunc(meta[cmds[1]])) {
				msg.Option(ice.MSG_HANDLE, ice.TRUE)
		...
		PushButton: function(button) { var args = can.core.List(arguments)
			msg.Push(ctx.ACTION, can.core.List(args, function(button) {
				return button && can.page.Format(html.INPUT, "", mdb.TYPE, html.BUTTON, mdb.NAME, button, mdb.VALUE, can.user.trans(can, button))
			}).join("")); return msg
		},
	...
	Inputs: function(can, msg, cmds, cb, meta) { var event = msg._event
		if (!cmds || cmds[0] != ctx.ACTION) { return false }
		var action = meta.feature[cmds[1]]
		if (!action && cmds[1] == mdb.MODIFY && meta.feature[mdb.CREATE]) {
			action = meta.feature[mdb.CREATE]
		}
		if (!action) { return false } msg.Option(ctx.ACTION, cmds[1])
		if (can.base.isFunc(action)) { cb = cb||function() { can.Update() }
		...
		var input = can.user.input(event, can, action, function(args) { msg.result = []
	...
	POST: function(can, msg, url, form, cb, cbs) { var xhr = new XMLHttpRequest(), begin = new Date(); msg._xhr = xhr, xhr._begin = begin
		var data = can.core.ItemForm(form, function(v, i, k) {
			if (msg._method != http.POST) { if (can.base.endWith(k, "_content")) { return } }
			if (msg._method != http.POST) { if (can.base.endWith(k, "content")) { return } }
			return k+mdb.EQ+encodeURIComponent(v)
		}).join("&")
		...
		xhr.open(msg._method||http.POST, url), xhr.onreadystatechange = function() { if (xhr.readyState != 4) { return }
			...
			var info = can.core.List(msg.result, function(res) { return can.user.trans(can, res) }).join("")+"\n"+msg.Option(ice.LOG_TRACEID)
			can.user.toastFailure(msg._can||can, info, msg.Option(ice.MSG_TITLE)), can.misc.Warn(xhr.status, res, url, form), cbs && cbs(xhr)
	...
	ResourceIcons: function(can, path, space, serve) {
		if (path == "1") { path = "usr/icons/avatar.jpg" }
		if (path == "9") { path = "usr/icons/avatar.jpg" }
		if (can.base.beginWith(path, web.HTTP)) { return path }
		if (can.base.beginWith(path, "bi bi")) { return path }
		if (!path) {
	...
	MergeURL: function(can, obj, clear) { var path = location.pathname; obj._path && (path = obj._path), delete(obj._path)
		can.misc.Search(can, log.DEBUG) && (obj.debug = obj.debug||ice.TRUE); var hash = obj._hash||""; delete(obj._hash)
		...
		obj = can.base.Copy(clear? {}: can.misc.ParseURL(can), obj), obj.debug == "false" && delete(obj.debug), delete(obj.pod), delete(obj.cmd)
		return can.base.MergeURL(_location.origin+(args.length > 0? nfs.PS+args.join(nfs.PS): path), obj)+(hash? "#"+hash: "")
	},
	ParseURL: function(can, url) {
		url = url||location.href; var args = can.base.ParseURL(url)
		...
		if (ls[0] == "s") { args[ice.POD] = decodeURIComponent(ls[1]); if (ls[2] == "c") {
			args[ice.CMD] = ls[3]
			for (var i = 4; i < ls.length; i++) {
				if (!ls[i]) { break }
				args[can.core.Item(can.Option())[i-4]] = ls[i]
			}
		} }
	...
	SearchHash: function(can) { var hash = location.hash
		if (arguments.length > 1) {
			hash = can.core.List(arguments, function(item) {
				if (can.base.isArray(item)) { return item.join(":") }
				return can.base.replaceAll(item, ":", "%3A")
			}).slice(1).join(":")
			hash = can.base.trimSuffix(hash, ":")
