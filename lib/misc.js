Volcanos("misc", {
	...
	Message: function(event, can) { var msg = kit.proto({}, {_event: event, _can: can, _target: can._target,
		RunAction: function(event, sub, cmds, meta, cb) { var msg = can.request(event); meta = meta || sub&&sub.onaction || {}
			if (!msg._method) {
			...
			if (msg.Option(ice.MSG_HANDLE) != ice.TRUE && cmds && cmds[0] == ctx.ACTION && can.base.isFunc(meta[cmds[1]])) { msg.Option(ice.MSG_HANDLE, ice.TRUE)
				return can.core.CallFunc(meta[cmds[1]], {event: event._event||event, can: sub, msg: msg, button: cmds[1], cmd: cmds[1], cb: cb}), true
			}
		},
	...
	Inputs: function(can, msg, cmds, cb, meta) { var event = msg._event
		if (cmds && cmds[0] == ctx.ACTION && meta.feature[cmds[1]]) { msg.Option(ctx.ACTION, cmds[1])
	...
	POST: function(can, msg, url, form, cb, cbs) { var xhr = new XMLHttpRequest(), begin = new Date(); msg._xhr = xhr, xhr._begin = begin
		var data = can.core.ItemForm(form, function(v, i, k) {
			if (msg._method != html.POST) { if (can.base.endWith(k, "_content")) { return } }
